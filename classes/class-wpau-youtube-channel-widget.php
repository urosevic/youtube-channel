...
class WPAU_YOUTUBE_CHANNEL_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		$handle   = ! empty( $instance['handle'] ) ? esc_attr( $instance['handle'] ) : $defaults['handle'];
		$vanity   = ! empty( $instance['vanity'] ) ? esc_attr( $instance['vanity'] ) : $defaults['vanity'];
		$channel  = ! empty( $instance['channel'] ) ? esc_attr( $instance['channel'] ) : $defaults['channel'];
		$username = ! empty( $instance['username'] ) ? esc_attr( $instance['username'] ) : $defaults['username'];
		$playlist = ! empty( $instance['playlist'] ) ? esc_attr( $instance['playlist'] ) : $defaults['playlist'];
		...
		<div class="halfs">
			<p class="glue-top">
				<label for="<?php echo $this->get_field_id( 'channel' ); ?>">
					...
					<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'channel' ); ?>" name="<?php echo $this->get_field_name( 'channel' ); ?>" value="<?php echo $channel; ?>" title="<?php _e( 'Channel ID starts with UC', 'youtube-channel' ); ?>" />
					<span class="description">
						<?php
						printf(
							wp_kses(
								// translators: %s is replaced with URL to YouTube Advanced Settings page
								__(
									'Get it from <a href="%s" target="_blank">here</a>',
									'youtube-channel'
								),
								array(
									'a' => array(
			...
			<p class="glue-top">
				<label for="<?php echo $this->get_field_id( 'playlist' ); ?>">
					...
					<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'playlist' ); ?>" name="<?php echo $this->get_field_name( 'playlist' ); ?>" value="<?php echo $playlist; ?>" title="<?php _e( 'Playlist starts with PL', 'youtube-channel' ); ?>" />
					<span class="description">
						<?php
						printf(
							wp_kses(
								// translators: %s is replaced with URL to user's YouTube Playlists page
								__(
									'Get it from <a href="%s" target="_blank">here</a>',
									'youtube-channel'
								),
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'handle' ); ?>">
				<?php _e( 'YouTube handle', 'youtube-channel' ); ?>
				<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'handle' ); ?>" name="<?php echo $this->get_field_name( 'handle' ); ?>" value="<?php echo $handle; ?>" title="<?php _e( 'Playlist starts with PL', 'youtube-channel' ); ?>" />
				<span class="description">
					<?php
					printf(
						wp_kses(
							// translators: %s is replaced with URL to YouTube handle page
							__(
								'Get it from <a href="%s" target="_blank">here</a>',
								'youtube-channel'
							),
		...
		<div class="halfs">
			<p class="glue-top deprecated">
				<label for="<?php echo $this->get_field_id( 'vanity' ); ?>">
					<?php _e( 'Vanity/Custom ID (deprecated)', 'youtube-channel' ); ?>
					<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'vanity' ); ?>" name="<?php echo $this->get_field_name( 'vanity' ); ?>" value="<?php echo $vanity; ?>" title="<?php _e( 'Legacy YouTube Vanity/Custom ID from URL (part after /c/)', 'youtube-channel' ); ?>" />
				</label>
			</p>
			<p class="glue-top deprecated">
				<label for="<?php echo $this->get_field_id( 'username' ); ?>">
					<?php _e( 'Legacy Username (deprecated)', 'youtube-channel' ); ?>
					<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'username' ); ?>" name="<?php echo $this->get_field_name( 'username' ); ?>" value="<?php echo $username; ?>" title="<?php _e( 'Legacy YouTube username located behind /user/ part of channel URL (available only on very old YouTube accounts)', 'youtube-channel' ); ?>" />
				</label>
			</p>
		</div><!-- .halfs -->
		...
		<div class="checkboxes">
			<p>
				<label for="<?php echo $this->get_field_id( 'privacy' ); ?>">
					...
					<?php
						// translators: %s is replaced with link to YouTube Enhanced Privacy article
						printf( __( 'Enable <a href="%s" target="_blank">privacy-enhanced mode</a>', 'youtube-channel' ), 'https://support.google.com/youtube/bin/answer.py?hl=en-GB&answer=171780' );
					?>
				</label>
		...
		<p class="glue-top">
			<label for="<?php echo $this->get_field_id( 'link_to' ); ?>">
				...
				<select class="widefat" id="<?php echo $this->get_field_id( 'link_to' ); ?>" name="<?php echo $this->get_field_name( 'link_to' ); ?>" onchange="ytcToggle('link_to', '<?php echo $this->get_field_id( '' ); ?>');" >
					...
					<option value="handle"<?php selected( $link_to, 'handle' ); ?>><?php _e( 'Link to Handle URL', 'youtube-channel' ); ?></option>
					<option value="channel"<?php selected( $link_to, 'channel' ); ?>><?php _e( 'Link to Channel URL', 'youtube-channel' ); ?></option>
					<option value="vanity"<?php selected( $link_to, 'vanity' ); ?>><?php _e( 'Link to Vanity URL (deprecated)', 'youtube-channel' ); ?></option>
					<option value="legacy"<?php selected( $link_to, 'legacy' ); ?>><?php _e( 'Link to Legacy username URL (deprecated)', 'youtube-channel' ); ?></option>
				</select>
			</label>
		</p>
		<p class="glue-top">
		...
		<p class="glue-top">
			<label for="<?php echo $this->get_field_id( 'goto_txt' ); ?>" <?php echo 'none' === $link_to ? 'class="hidden"' : ''; ?>>
				...
				<input class="widefat" id="<?php echo $this->get_field_id( 'goto_txt' ); ?>" name="<?php echo $this->get_field_name( 'goto_txt' ); ?>" type="text" value="<?php echo $goto_txt; ?>" title="<?php /* translators: %1$s is replaced with placeholder for vanity, %2$s for channel and %3$s for username */ sprintf( __( 'Default: Visit our YouTube channel. You can use placeholders %1$s, %2$s and %3$s.', 'youtube-channel' ), '%vanity%', '%channel%', '%username%' ); ?>" placeholder="<?php _e( 'Visit our YouTube channel', 'youtube-channel' ); ?>" />
			</label>
		</p>
		...
		<p class="description">
			...
			printf(
				wp_kses(
					/* translators: %1$s is replaced with URL to plugin official FAQ page
					* %2$s with translated label FAQ
					* %3$s with URL to community forum
					*/
					__(
						'Carefully read <a href="%1$s" target="_faq">%2$s</a> before you contact us on <a href="%3$s" target="_support">community support forum</a>.',
						'youtube-channel'
					),
					array(
						'a' => array(
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['link_to']        = isset( $new_instance['link_to'] ) && in_array( $new_instance['link_to'], array( 'none', 'handle', 'vanity', 'channel', 'legacy' ), true ) ? $new_instance['link_to'] : $old_instance['link_to']; // string
